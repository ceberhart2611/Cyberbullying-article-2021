...
with pyodbc.connect('DRIVER='+driver+';Server=(LocalDb)\MSSQLLocalDB;Integrated Security=true;Database=bullyingdb;') as conn:
    with conn.cursor() as cursor:
        cursor.execute("select distinct(A.commentid), doc_positive_confidence, doc_neutral_confidence, doc_negative_confidence, linkpresence, doccursing, docgeneral, doccontext, docsexlang, manual_judgement from LR_allfeatures A join manualjudgement M on A.commentid = M.commentid join above30_9010 N on A.commentid = N.commentid where recordtype = 'training'")
...
with conn.cursor() as cursor:
        cursor.execute("select distinct(A.commentid), doc_positive_confidence, doc_neutral_confidence, doc_negative_confidence, linkpresence, doccursing, docgeneral, doccontext, docsexlang, manual_judgement from LR_allfeatures A join manualjudgement M on A.commentid = M.commentid join above30_9010 N on A.commentid = N.commentid where recordtype = 'testing'")
...
for trecord in test_records:
    ...
    with conn.cursor() as cursor:
        cursor.execute("insert into resNKP_9010 values (" + str(trecord.commentid) + ", NULL, '" + str(jud) + "')")
...
for trecord in test_records:
    ...
    with conn.cursor() as cursor:
        cursor.execute("update resNKP_9010 set NBjudg = '" + str(jud) + "' where commentid = " + str(trecord.commentid) )
