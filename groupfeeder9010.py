...
for current_comment in all_comments:
    if i<2:
        with conn.cursor() as cursor:
            cursor.execute("insert into above30_9010 values (" + str(current_comment.commentid) + ", 'testing')")
        i=i+1
    elif i<10:
        with conn.cursor() as cursor:
            cursor.execute("insert into above30_9010 values (" + str(current_comment.commentid) + ", 'training')")
        i=i+1
    elif i==10:
        with conn.cursor() as cursor:
            cursor.execute("insert into above30_9010 values (" + str(current_comment.commentid) + ", 'training')")
